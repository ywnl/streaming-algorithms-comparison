...
def evaluate_top_k(true_counts, estimated_counts, k=10):
...
def benchmark_all_algos(stream, true_counts, memory_config):
    ...
    results["MisraGries"] = {
        ...
        "topk": evaluate_top_k(true_counts, mg_est, k=10)
    }
    ...
    results["CountMinSketch"] = {
        ...
        "topk": evaluate_top_k(true_counts, cms_est, k=10)
    }

    ### Count Sketch
    start = time.time()
    cs = BasicCountSketch(width=width)
    for item in stream:
        cs.update(item)
    ...
    results["CountSketch"] = {
        ...
        "topk": evaluate_top_k(true_counts, cs_est, k=10)
    }
    ...
    results["CountMedian"] = {
        ...
        "topk": evaluate_top_k(true_counts, cm_est, k=10)
    }
